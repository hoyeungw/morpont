...
import { samplesToCrostab, samplesToTable } from '@analys/convert';
import { MUTABLE } from '@analys/enum-mutabilities';
import { TABLE } from '@analys/enum-tabular-types';
import { DictCollection } from '@glossa/abbr-fin';
import { BALANCES, SYMBOL, INCOMES, CASHFLOWS, GENERALS } from '@glossa/enum-fin';
import { camelToSnake, snakeToCamel } from '@spare/phrasing';
import { makeReplaceable, Translator } from '@spare/translator';
import { init, iso as iso$1 } from '@vect/vector-init';
  ...
  static async timeseries({
    ...
    return await Acq.tabular({
      ...
      prep: ({
        ...
        const table = (_quarterlyReports = quarterlyReports, samplesToTable(_quarterlyReports));
        _ref = (_table$head$map = table.head.map(x => camelToSnake(x, ' ')), decoVector(_table$head$map)), logger(_ref);
        const translatorB = Translator.build(merges(AbbrAlphav['grammatic'], AbbrAlphav[report], DictCollection[report]));
        table.mapHead(x => translatorB.parse(camelToSnake(x, ' '), snakeToCamel), MUTABLE).find({
          date: date => within(date, start, end)
        }).unshiftColumn(SYMBOL, iso$1(table.ht, symbol)).setTitle(symbol);
