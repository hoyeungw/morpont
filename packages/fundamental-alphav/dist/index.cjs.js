...
const FieldsCrostab = crostab.CrosTab.from({
  side: [enumFin.BALANCES, enumFin.INCOMES, enumFin.CASHFLOWS],
  head: [enumDataScopes.DB, enumDataScopes.LITE],
...
class FundamentalAlphav {
  ...
  static async timeseries({
    symbol = 'AAPL',
    start = dashedDate.shiftYear(TODAY, -3),
    end = TODAY,
    report = enumFin.BALANCES,
    format = enumTabularTypes.TABLE,
    scope = enumDataScopes.LITE
  } = {}) {
    return await acq.Acq.tabular({
      ...
      prep: ({
        ...
        const table = convert.samplesToTable(quarterlyReports, FieldsCrostab.cell(report, scope));
        table.find({
          date: date => dashedDate.within(date, start, end)
        }).mutate(x => x / MILLION, {
          exclusive: [enumFin.SYMBOL, enumFin.DATE, 'curr']
        }).unshiftColumn(enumFin.SYMBOL, vectorInit.iso(table.ht, symbol)).setTitle(symbol);
        return table;
      },
      args: {
        symbol,
...
exports.FieldsCrostab = FieldsCrostab;
